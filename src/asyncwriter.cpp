...
// AsyncWriter::Stop() — Signal the background thread to exit, then wait for it.
// We wait indefinitely (INFINITE) because the thread's only blocking operation
// is WriteFileAtomic which always terminates. If something truly hangs we let
// the OS kill the process rather than risk use-after-free by releasing sync
// objects while the thread is still alive.
void AsyncWriter::Stop() {
    ...
    // Wait for thread to actually exit — no timeout, no handle release on failure
    DWORD wait = WaitForSingleObject(thread_, INFINITE);
    (void)wait;
