...
bool MigrateDataDir(bool toPortable, HWND owner) {
    ...
    // If destination already has any of the files we'll move, confirm overwrite
    const wchar_t* files[] = {L"config.ini", L"store.dat", L"clipwiz.log"};
    bool dstHasConflict = false;
    for (const wchar_t* f : files) {
        std::wstring dstFile = dst + L"\\" + f;
        if (GetFileAttributesW(dstFile.c_str()) != INVALID_FILE_ATTRIBUTES) {
            dstHasConflict = true;
            break;
        }
    }
    ...
    // Close logger so clipwiz.log handle is released before moving
    logger::Shutdown();
    int result = SHFileOperationW(&op);

    if (result != 0 || op.fAnyOperationsAborted) {
        // Move failed — reinit logger at old location, keep source dir
        logger::Init();
        return false;
    }
    g_dataDir = dst;
    // Reopen logger at new data directory
    logger::Init();
    return true;
}
